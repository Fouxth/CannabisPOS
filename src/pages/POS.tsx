...
import { Search, Grid3X3, List, Plus, Minus, Trash2, Percent, Receipt, CreditCard, Banknote, QrCode, ArrowRightLeft, Box, ShoppingCart, TrendingUp, Award } from 'lucide-react';
      ...
      <Card className={cn(
        ...
        <div className="px-4 pb-3 border-b border-border/50 bg-muted/20 space-y-2">
          ...
          {enablePoints && (!selectedCustomer ? (
            ...
            <div className="p-2.5 rounded-lg bg-emerald-500/10 border border-emerald-500/20 text-xs space-y-2">
              <div className="flex items-center justify-between">
                <div>
              ...
              {/* 10-Stamp Card Progress */}
              <div className="pt-1.5 border-t border-emerald-500/20">
                <div className="flex items-center justify-between mb-1">
                  <span className="text-[10px] font-semibold text-emerald-700 dark:text-emerald-300">
                    บัตรสะสมแต้ม (ครบ 10 แต้มรับรางวัล)
                  </span>
                  {selectedCustomer.points >= 10 && (
                    <Button
                      size="sm"
                      variant="default"
                      className="h-5 text-[10px] px-2 bg-emerald-600 hover:bg-emerald-700 text-white font-bold animate-pulse"
                      onClick={() => {
                        if (pointsRedeemed === 0) {
                          setPointsRedeemed(10);
                          toast.success('ใช้ 10 แต้มแลกส่วนลด/ของรางวัลเรียบร้อย');
                        } else {
                          setPointsRedeemed(0);
                          toast.info('ยกเลิกใช้แต้มแลกรางวัล');
                        }
                      }}
                    >
                      {pointsRedeemed > 0 ? '✓ ใช้แลกแล้ว (-10)' : '🎁 แลกของรางวัล'}
                    </Button>
                  )}
                </div>
                {/* 10 Stamp Grid Slots */}
                <div className="grid grid-cols-10 gap-1 text-center">
                  {Array.from({ length: 10 }).map((_, idx) => {
                    ...
                    return (
                      <div
                        key={idx}
                        className={`h-6 rounded flex items-center justify-center text-[10px] font-bold border transition-colors ${
                          isCurrent
                            ? 'bg-emerald-500 text-white border-emerald-600 shadow-sm'
                            : isNew
                            ? 'bg-amber-400/30 text-amber-600 border-amber-400 border-dashed animate-pulse'
                            : 'bg-muted/40 text-muted-foreground border-border/40'
                        }`}
                        title={`แต้มที่ ${idx + 1}`}
                      >
                        {isCurrent ? '★' : isNew ? '+' : idx + 1}
                      </div>
                    );
                  })}
                </div>
